...
int main()
{
    ...
    std::filesystem::path textPath{pathUtils::make_absolute_path("resources/models/candy-text.png")};
    Texture2D candyCaneText = LoadTexture(textPath.string().c_str()); // Load model texture and set material
    context.candyCane.materials[0].maps[MATERIAL_MAP_DIFFUSE].texture = candyCaneText;
