...
void generateHeightmap(AppContext &context)
{
    ...
    context.heightmapImage = GenImageFromNoiseFunction<float>(resolution, resolution, PIXELFORMAT_UNCOMPRESSED_R32,
                                                              [&](glm::vec2 const &p) -> float
                                                              {
                                                                  ...
                                                                  return (octaveNoise(p, perlinNoise, context.octaves, context.lacunarity, context.gain, context.imageGenerationParameters.noiseScale) * 0.5f + 0.5f) * masqueRadial(p, context.radius, context.fit);
                                                              });
